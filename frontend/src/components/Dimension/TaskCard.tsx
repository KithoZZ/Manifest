  ...
  return (
    <div 
      ...
      style={{ 
        ...
        borderRadius: isMinimized ? '12px' : '20px', 
        padding: isMinimized ? '12px 20px' : '28px', 
        borderLeft: `4px solid ${statusColor}`,
        transition: 'all 0.35s ease-in-out',
        boxShadow: isMinimized ? '0 4px 12px rgba(0,0,0,0.06)' : '0 12px 32px rgba(0,0,0,0.08)',
        border: isMinimized ? `1px solid ${config.color}30` : '1px solid #f8f8f8',
        overflow: 'hidden',
        position: 'relative',
        background: isMinimized ? '#ffffff' : 'linear-gradient(145deg, #ffffff, #f8f9fa)',
        cursor: 'pointer'
      }}
      onMouseEnter={(e) => {
        if (isMinimized) {
